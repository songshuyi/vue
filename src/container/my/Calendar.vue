        ...
        <div class="calendar">
            ...
            <div class="day">
                <div v-for="(item,index) in days">
                    <p :tip="tip[index-first+1]"
                       v-text="item.val"
                       :class="[check_day === index ? 'check_day': '',item.color,item.tip]"
                       @click="judge(item.type,index)"></p>
                </div>
            </div>
        </div>
        <div class="list">
            <div class="title">2017年6月22日</div>
            <div class="detail" v-show="tip[check_day - first + 1 || today]">
                <div class="name">众金宝0245号</div>
                <div class="money">
                    <p>第一期回款金额(元)</p>
                    <p>234</p>
                </div>
...
<style lang="scss" scoped>
    ...
    .calendar{
        ...
        .day{
            @include box((d:flex,fs:0.32rem,c:$black2));flex-wrap: wrap;
            div{
